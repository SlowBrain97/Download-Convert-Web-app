...
async function tryLibreOfficeConvert(buffer, extOut) {
    try {
        // dynamic import optional dependency
        return await new Promise((resolve, reject) => {
            libreofficeConvert.convert(buffer, `.${extOut}`, undefined, (err, done) => {
                if (err)
                    reject(err);
                else
                    resolve(done);
...
export async function convertDocTask(taskId, inputPath, inputFormat, outputFormat) {
    try {
